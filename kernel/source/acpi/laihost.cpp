...
void *laihost_map(size_t addr, size_t bytes){
    uint64_t n_pages = misc::div_ceil(bytes, mm::pmm::block_size); // Ceil it to make sure we have all bytes allocated incase (bytes & 0xFFF) != 0
...
void laihost_unmap(size_t, size_t){
